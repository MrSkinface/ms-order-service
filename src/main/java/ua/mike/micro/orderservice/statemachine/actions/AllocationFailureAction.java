...
/**
 * Created by mike on 01.06.2022 20:44
 */
@Component
@Slf4j
public class AllocationFailureAction implements Action<OrderStatus, OrderEvent> {
    ...
    public void execute(StateContext<OrderStatus, OrderEvent> context) {
        Optional.ofNullable(context.getMessage().getHeaders().get(ORDER_ID_HEADER, String.class)).ifPresent(id -> {
            log.debug("AllocationFailureAction for id: {}", id);
        });
    }
}
